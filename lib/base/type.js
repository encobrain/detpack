...
function getIndex (bitsPerIndex) {
    ...
    if (this.indexShift > 7) {
        this.indexShift &= 7;

        if (this.indexByte != null) {
            var index = this.indexByte << (bitsPerIndex - this.indexShift);
            this.indexByte = this[this.offset++];

            return (index | this.indexByte >> (8-this.indexShift) ) & bitsToMask[bitsPerIndex];
        }

        this.indexByte = this[this.offset++];
    }

    return (this.indexByte >> (8-this.indexShift)) & bitsToMask[bitsPerIndex];
}
