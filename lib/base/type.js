...
function addIndex (index, bitsPerIndex) {
    ...
    if (this._indexBits > 7) {
        this._indexBits &= 7;

        if (this._indexOffset != null) {
            this[this._indexOffset] = (this._indexByte << this._indexBits) | (index >> (bitsPerIndex - this._indexBits));
        }
...
function getBuffer (value) {
    var buffer = new Buffer(Math.ceil(this.bitsLength(value) / 8));
