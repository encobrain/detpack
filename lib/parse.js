...
function parse (text) {
    ...
    function getNumber () {
        ...
        if (c === '0') {
            c = text[++i];
            switch (c) {
                case 'x': c=text[++i]; is = isHex; break;
                case 'b': c=text[++i]; is = isBin; break;
                case 'o': c=text[++i]; is = isOct; break;
                default: i--;
            }
        }
    ...
    while (i < l) {
        ...
        switch (c) {
            case '{': parseObject(name); break;
            case '[': parseOneof(name); break;
