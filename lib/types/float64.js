...
function encodeList (numbers, buffer) {
    ...
    if (length < this._listMin)
        throw new Error('Required minimum ' + this._listMin + ' Float64 numbers');

    if (length > this._listMax)
        throw new Error('Allow maximum ' + this._listMax + ' Float64 numbers');
...
function checkDefaultList (numbers, propDesc) {
    ...
    if (length < this._listMin)
        throw new Error('Incorrect default. Required minimum ' + this._listMin + ' Float64 numbers in ' + propDesc);

    if (length > this._listMax)
        throw new Error('Incorrect default. Allow maximum ' + this._listMax + ' Float64 numbers in ' + propDesc);
}
...
Float64Type.prototype._listMin = null;
Float64Type.prototype._listMax = null;
