...
function encodeList (numbers, buffer) {
    ...
    if (length < this._listMin)
        throw new Error('Required minimum ' + this._listMin + ' Int16 numbers');

    if (length > this._listMax)
        throw new Error('Allow maximum ' + this._listMax + ' Int16 numbers');
...
function checkDefaultList (numbers, propDesc) {
    ...
    if (length < this._listMin)
        throw new Error('Incorrect default. Required minimum ' + this._listMin + ' Int16 numbers in ' + propDesc);

    if (length > this._listMax)
        throw new Error('Incorrect default. Allow maximum ' + this._listMax + ' Int16 numbers in ' + propDesc);
...
Int16Type.prototype._listMin = null;
Int16Type.prototype._listMax = null;
