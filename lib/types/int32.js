...
function encodeList (numbers, buffer) {
    ...
    if (length < this._listMin)
        throw new Error('Required minimum ' + this._listMin + ' Int32 numbers');

    if (length > this._listMax)
        throw new Error('Allow maximum ' + this._listMax + ' Int32 numbers');
...
function checkDefaultList (numbers, propDesc) {
    ...
    if (length < this._listMin)
        throw new Error('Incorrect default. Required minimum ' + this._listMin + ' Int32 numbers in ' + propDesc);

    if (length > this._listMax)
        throw new Error('Incorrect default. Allow maximum ' + this._listMax + ' Int32 numbers in ' + propDesc);
...
function decode (buffer, inited) {
    ...
    var number = buffer.readInt32LE(buffer._offset, true);
    buffer._offset += 4;
...
Int32Type.prototype._listMin = null;
Int32Type.prototype._listMax = null;
