...
function encode (bin, buffer) {
    ...
    if (Buffer.isBuffer(bin)) buffer._offset += bin.copy(buffer, buffer._offset, 0, bl);
    else buffer._offset += buffer.write(bin, buffer._offset, bl);
...
function encodeList (bins, buffer) {
    ...
    if (length < this._listMin)
        throw new Error('Incorrect Bin list. Required minimum ' + this._listMin + ' Bin values');

    if (length > this._listMax)
        throw new Error('Incorrect Bin list. Allow maximum ' + this._listMax + ' Bin values');
...
function checkDefaultList (bins, propDesc) {
    var length = bins.length, len;

    if (length < this._listMin)
        throw new Error('Incorrect Bin default. Required minimum ' + this._listMin + ' Bin values in ' + propDesc);

    if (length > this._listMax)
        throw new Error('Incorrect Bin default. Allow maximum ' + this._listMax + ' Bin values in ' + propDesc);

    while (length--) {
        len = Buffer.byteLength(bins[length]);
...
function decode (buffer, inited) {
    ...
    if (bl) buffer._offset += buffer.copy(bin, 0, buffer._offset, buffer._offset + bl);
...
BinType.prototype._listMin = null;
BinType.prototype._listMax = null;
