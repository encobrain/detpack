...
function encodeList (bools, buffer) {
    ...
    if (length < this._listMin)
        throw new Error('Required minimum ' + this._listMin + ' Bool values');

    if (length > this._listMax)
        throw new Error('Allow maximum ' + this._listMax + ' Bool values');
...
function checkDefaultList (numbers, propDesc) {
    ...
    if (length < this._listMin)
        throw new Error('Incorrect default. Required minimum ' + this._listMin + ' Bool values in ' + propDesc);

    if (length > this._listMax)
        throw new Error('Incorrect default. Allow maximum ' + this._listMax + ' Bool values in ' + propDesc);
}

function decode (buffer, inited) {
    if (!inited) this.initBuffer(buffer);

    return buffer._getIndex(1) ? true : false;
}
...
BoolType.prototype._listMin = null;
BoolType.prototype._listMax = null;
