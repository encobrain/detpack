...
function encodeList (numbers, buffer) {
    ...
    if (length < this._listMin)
        throw new Error('Required minimum ' + this._listMin + ' Uint16 numbers');

    if (length > this._listMax)
        throw new Error('Allow maximum ' + this._listMax + ' Uint16 numbers');
...
function checkDefaultList (numbers, propDesc) {
    ...
    if (length < this._listMin)
        throw new Error('Incorrect default. Required minimum ' + this._listMin + ' Uint16 numbers in ' + propDesc);

    if (length > this._listMax)
        throw new Error('Incorrect default. Allow maximum ' + this._listMax + ' Uint16 numbers in ' + propDesc);
...
Uint16Type.prototype._listMin = null;
Uint16Type.prototype._listMax = null;
