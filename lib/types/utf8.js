...
function encode (str, buffer) {
    if (typeof str !== 'string')
        throw new Error('Incorrect Utf8 string');

    if (!buffer) buffer = this.getBuffer(str);

    if (str.length < this._lenMin)
        throw new Error('Incorrect Utf8 string. Required minimum ' + this._lenMin + ' chars');

    if (str.length > this._lenMax)
        throw new Error('Incorrect Utf8 string. Allow max ' + this._lenMax + ' chars');
...
function encodeList (strs, buffer) {
    ...
    if (length < this._listMin)
        throw new Error('Incorrect Utf8 list. Required minimum ' + this._listMin + ' Utf8 strings');

    if (length > this._listMax)
        throw new Error('Incorrect Utf8 list. Allow maximum ' + this._listMax + ' Utf8 strings');
...
function checkDefault (str, propDesc) {
    ...
    if (str.length < this._lenMin)
        throw new Error('Incorrect default. Required minimum ' + this._lenMin + ' chars in ' + propDesc);

    if (str.length > this._lenMax)
        throw new Error('Incorrect default. Allow max ' + this._lenMax + ' chars in ' + propDesc);
}
...
function checkDefaultList (strs, propDesc) {
    ...
    if (length < this._listMin)
        throw new Error('Incorrect Bin default. Required minimum ' + this._listMin + ' Utf8 strings in ' + propDesc);

    if (length > this._listMax)
        throw new Error('Incorrect Bin default. Allow maximum ' + this._listMax + ' Utf8 strings in ' + propDesc);
    ...
    while (length--) {
        ...
        if (len < this._lenMin)
            throw new Error('Incorrect Bin default. Required minimum ' + this._lenMin + ' chars in ' + propDesc + '['+length+']');

        if (len > this._lenMax)
            throw new Error('Incorrect Bin default. Allow max ' + this._lenMax + ' chars in ' + propDesc + '['+length+']');
    }
}
...
function setOpts (opts, propDesc) {
    ...
    this._lenMax = max || Number.MAX_VALUE;
}
...
Utf8Type.prototype._lenMin = 0;
Utf8Type.prototype._lenMax = Number.MAX_VALUE;
Utf8Type.prototype._listMin = null;
Utf8Type.prototype._listMax = null;
