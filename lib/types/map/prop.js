...
function setCompilerOptions (options) {
    ...
    function setDefaultOpts (opt, parent) {
        if (opt instanceof CompilerOption) {
            if (opt.name === 'null') return null;
            if (opt.name === 'true') return true;
            if (opt.name === 'false') return false;
    ...
    if (self.default) self.default = { value: setDefaultOpts(self.default, self.structName+'.'+self.name + '.default') };
}
