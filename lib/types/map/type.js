...
function createEncodeFn (props) {
    ...
    fn('    if (arguments.length === 1) buffer.fixIndex();')
      ('    return buffer;')
      ('}');
...
function createDecodeFn (props) {
    ...
    function add (prop, i) {
        ...
        if (prop.defaultValue) {
            fn('%s : buffer.getIndex(1) ?', prop.name)
...
MapType.prototype.encode = encode;
MapType.prototype.decode = decode;
MapType.prototype.bitsLength = bitsLength;
