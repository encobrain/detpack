...
var typeId = require('../base/type-id'),
...
function NumType () {
    BaseType.call(this, typeId.Num, 'Num');
}
...
function decode (buffer, inited) {
    ...
    switch (type) {
        ...
        case 0x2:
            number = buffer.readUInt32LE(buffer._offset, false);
            buffer._offset += 4;
            break;

        case 0x3:
            number = buffer.readUInt32LE(buffer._offset + 4, false);
        ...
        case 0x6:
            number = buffer.readInt32LE(buffer._offset, false);
            buffer._offset +=4;
            break;

        case 0x7:
            number =
                buffer.readInt32LE(buffer._offset + 4, false) * HALF +
                buffer.readUInt32LE(buffer._offset, false);
            buffer._offset += 8;
            break;
        ...
        case 0x9:
            number = buffer.readFloatLE(buffer._offset, false);
            buffer._offset += 4;
            break;

        case 0xA:
            number = buffer.readDoubleLE(buffer._offset, false);
            buffer._offset += 8;
            break;
...
function decodeList (buffer, inited) {
    ...
    while (k < length) {
        switch (type = buffer._getIndex(4)) {
            case 0x0:
                number = buffer[buffer._offset++];
                break;
