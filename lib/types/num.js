...
function encode (number, buffer) {
    ...
    if (number != number) buffer.addIndex(0xE, 4);
    else if (number === Infinity) buffer.addIndex(0xC, 4);
    else if (number === -Infinity) buffer.addIndex(0xD, 4);
    else if (number % 1) {
        buffer.addIndex(0xA, 4);
        buffer.offset = buffer.writeDoubleLE(number, buffer.offset, true);
    } else if (number < 0) {
        if (number > -0x80)  {
            buffer.addIndex(0x4, 4);
            buffer[buffer.offset++] = number & 0xFF;
        } else if (number > -0x8000) {
            buffer.addIndex(0x5, 4);
            buffer[buffer.offset++] = number & 0xFF;
            buffer[buffer.offset++] = number >> 8 && 0xFF;
        } else if (number > -0x80000000) {
            buffer.addIndex(0x6, 4);
            buffer.offset = buffer.writeInt32LE(number, buffer.offset, true);
        } else if (number > MIN_INT64){
            buffer.addIndex(0x7, 4);
        ...
        if (number < 0x100) {
            ...
            buffer.addIndex(2,4);
            buffer.offset = buffer.writeUInt32LE(number, buffer.offset, false);
...
function decode (buffer, inited) {
    ...
    switch (buffer.getIndex(4)) {
        case 0x0:
            return buffer[buffer.offset++];
        case 0x1:
            number = buffer[++buffer.offset] << 8;

            return number + buffer[buffer.offset++ - 1];
        case 0x2:
            number = buffer.readUInt32LE(buffer.offset, true);
            buffer.offset += 4;

            return number;
        case 0x3:
            number = buffer.readUInt32LE(buffer.offset + 4, true);
            number = number * HALF + buffer.readUInt32LE(buffer.offset, true);
            buffer.offset += 8;
            return number;
        case 0x4:
            number = buffer[buffer.offset++];
            return number > MAX_INT8 ? number-MAX_UINT8 : number;
        case 0x5:
            number = buffer[++buffer.offset] << 8;
            number = number + buffer[buffer.offset++ - 1];

            return number > MAX_INT16 ? number-MAX_UINT16 : number;
        case 0x6:
            number = buffer.readInt32LE(buffer.offset, true);
            buffer.offset +=4;
            return number;
        case 0x7:
            number = buffer.readUInt32LE(buffer.offset + 4, true);
        ...
        case 0x9:
            number = buffer.readFloatLE(buffer.offset, true);
            buffer.offset += 4;
            return number;
        case 0xA:
            number = buffer.readDoubleLE(buffer.offset, true);
            buffer.offset += 8;
            return number;
...
function decodeList (buffer, inited) {
    ...
    while (k < length) {
        switch (buffer.getIndex(4)) {
            case 0x0:  // uint8
                numbers[k] = buffer[buffer.offset++];
                break;
            case 0x1:  // uint16
                number = buffer[++buffer.offset] << 8;

                numbers[k] = number + buffer[buffer.offset++ - 1];
                break;
            case 0x2:  // uint32
                numbers[k] = buffer.readUInt32LE(buffer.offset, true);
                buffer.offset += 4;
                break;
            case 0x3:  // uint64
                number = buffer.readUInt32LE(buffer.offset + 4, true);
                numbers[k] = number * HALF + buffer.readUInt32LE(buffer.offset, true);
                buffer.offset += 8;
                break
            case 0x4:  // int8
                number = buffer[buffer.offset++];
            ...
            case 0x5:  // int16
                ...
                break;
            case 0x6:   // int32
                numbers[k] = buffer.readInt32LE(buffer.offset, true);
                buffer.offset +=4;
                break
            case 0x7:   // int64
                number = buffer.readUInt32LE(buffer.offset + 4, true);
            ...
            case 0x9:
                numbers[k] = buffer.readFloatLE(buffer.offset, true);
                buffer.offset += 4;
                break;
            case 0xA:
                numbers[k] = buffer.readDoubleLE(buffer.offset, true);
                buffer.offset += 8;
                break;
...
NumType.prototype.encodedLength = encodedLength;
