...
function encode (value, buffer) {
    ...
    if (typeof i === 'object') i.encode(value, buffer);
...
function checkDefault (value, propDesc) {
    ...
    if (typeof v === 'object') v.checkDefault(value, propDesc);
}
...
function checkDefaultList (values, propDesc) {
    ...
    while (length--) {
        ...
        if (typeof v === 'object') v.checkDefault(value, propDesc + '.' + length);
    }
}
...
function decode (buffer, inited) {
    ...
    if (typeof v === 'object') v = v.decode(buffer, 1);
...
function decodeList (buffer, inited) {
    ...
    var k = 0,
        length = this._listUint.decode(buffer, 1),
...
function bitsLength (value) {
    ...
    return typeof v === 'object' ? this._indexBits + v.bitsLength(value) : this._indexBits;
}

function bitsLengthList (values) {
    var length = values.length,
        len = this._listUint.bitsLength(length) + length * this._indexBits,
        value, v;

    while (length--) {
        value = values[length];
        v = this._values[ this._indexFn(value) ];
        if (typeof v === 'object') len += v.bitsLength(value);
    }
