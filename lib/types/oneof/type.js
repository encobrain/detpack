...
function OneofType (name, indexToValue, staticToIndex, objectToIndex) {
...
function createBitsLengthListFn (oneof) {
    ...
    if (oneof._objectToIndex_code) {
        if (oneof._staticToIndex) {
            fn('if (value != null && typeof value === "object")')
              ('    len += indexToValue[ %s ].bitsLength(value);', oneof._indexBits, oneof._objectToIndex_code)
        } else {
            fn('len += indexToValue[ %s ].bitsLength(value);', oneof._indexBits, oneof._objectToIndex_code);
        }
    }
