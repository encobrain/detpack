...
function encode (value, buffer) {
    ...
    buffer._addIndex(i, this.indexBits);
...
function checkDefaultList (values) {
    ...
    if (length < this._listMin)
        throw new Error('Incorrect Oneof list default. Required minimum ' + this._listMin + ' Oneof values in ' + propDesc);

    if (length > this._listMax)
        throw new Error('Incorrect Oneof list. Allow maximum ' + this._listMax + ' Oneof values in ' + propDesc);
...
function decode (buffer, inited) {
    ...
    var v = this.values[ buffer._getIndex(this.indexBits) ];
...
function decodeList (buffer, inited) {
    ...
    while (k < length) {
        v = this.values[ buffer._getIndex(this.indexBits) ];
        if (v instanceof BaseType) v = v.decode(buffer, 1);
        values[k++] = v;
    }
...
OneofType.prototype._listMin = null;
OneofType.prototype._listMax = null;
