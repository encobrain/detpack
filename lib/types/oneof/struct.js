...
function compileStruct (types, options) {
    ...
    function collect (prop) {
        ...
        function compare (map) {
            ...
            function check (key) {
                ...
                if (typeof v === 'number') {
                    var vPropKeys = (new (types[self.props[v].name])()).requiredKeys,
                        next = key === '$' ? map : {};
    ...
    function values (prop) {
        return prop.type === 'STATIC' ? prop.value : new types[prop.name]();
    }

    var props = self.props.map(values);

    function OneofStructType () {
        return new OneofType(self.name, indexFn, props);
    }
